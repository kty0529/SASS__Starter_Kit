...
- **helpers**: 재사용 가능한 토큰과 믹스인을 제공하는 핵심 라이브러리 영역입니다.
  ...
  - `_mixins.scss`: `disable`, `text-ellipsis`, `box-ellipsis`, `hide-scrollbar`, `px-to-vw`, `container` 등 실무 유틸리티 믹스인을 모아두고 각 파라미터 기본값을 주석으로 안내합니다. 새 믹스인을 추가할 때도 이 파일에 문서화를 함께 남깁니다.
...
| `container`      | 공통 레이아웃 폭  | `$useGutter: "true"`                                  |
...
```scss
.copy {
  @include px-to-vw(font-size, 12px);
}
...
### `container($useGutter: "true")`

- `$container`와 `$gutter` 값을 활용해 공통 레이아웃 폭을 정의합니다. `$useGutter`를 `"false"`로 전달하면 내부 좌우 패딩 없이 최대 폭만 적용됩니다.
